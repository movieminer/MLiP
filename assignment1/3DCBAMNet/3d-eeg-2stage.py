...
class config:
    ...
    FC=66
    RATIO=32
    WEIGHT_DECAY=0.034774
    PCT_START=0.17305
    FINAL_DIV_FACTOR=96.983


class paths:
    OUTPUT_DIR = "./pop2/"
    TRAIN_CSV = "/vol/tensusers4/thijsdejong/input/dataset/train_csv_te.csv"
...
class EEGDatasetSplit(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.augment and self.mode == 'train':
            ...
            # Randomly shift time
            time_shift = random.randint(-int(eeg_length * 0.1), int(eeg_length * 0.1))
...
def train_loop(df, fold):    
    ...
    train_dataset1 = EEGDatasetSplit(train_folds[train_folds['total_evaluators'] < 10], eegs=train_eegs, mode="train", augment=True)
    train_dataset2 = EEGDatasetSplit(train_folds[train_folds['total_evaluators'] >= 10], eegs=train_eegs, mode="train", augment=True)
    
    valid_dataset = EEGDatasetSplit(valid_folds, eegs=train_eegs, mode="train", augment=False)
    ...
    valid_folds1, train_losses1, val_losses1 = train_stage(train_loader1, valid_loader, valid_folds, model, config.MAX_LR)
    valid_folds2, train_losses2, val_losses2 = train_stage(train_loader2, valid_loader, valid_folds, model, config.MAX_LR/2)
